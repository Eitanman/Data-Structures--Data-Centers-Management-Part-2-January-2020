...
private:
    ...
    Vertex addVertexAux(int server_num, int traffic, Vertex curr_root) {
        traffic_key temp_key(traffic, server_num);
        if (curr_root == nullptr) { // this means we've reached a leaf
            ...
            return curr_root;
        } else if (temp_key < *(curr_root->key)) { // v should be on the left of curr root, we have to overload the operator < for key*.
            curr_root->left_son = addVertexAux(server_num, traffic,
                                               curr_root->left_son);
        } else if (temp_key > *(curr_root->key)) {
...
public:
    ...
    void storeInorder(Vertex curr_root, Key array[], int *index_ptr)
    {
        if (curr_root == nullptr)
            return;

        /* first recur on left child */
        storeInorder(curr_root->left_son, array, index_ptr);

        array[*index_ptr] = new traffic_key(*(curr_root->key));
        (*index_ptr)++; // increase index for next entry

        /* now recur on right child */
        storeInorder(curr_root->right_son, array, index_ptr);
    }

    tree<Traffic_Key> * arrayToTree(Traffic_Key arr[], int start, int end){
        root=arrayToTreeAux(arr,start,end);
        return this;
    }

    Vertex arrayToTreeAux(Traffic_Key arr[], int start, int end)
    {
        /* Base Case */
        if (start > end)
            return nullptr;
        ...
        int mid = (start + end)/2;
        Vertex root = new vertex(arr[mid]->getServerNum(),arr[mid]->getTrafficNum());
        root->left_son = arrayToTreeAux(arr, start, mid-1);
        root->right_son = arrayToTreeAux(arr, mid+1, end);

        int curr_height = max(height(root->left_son), height(root->right_son)) + 1;
        root->height = curr_height;
        root->sub_tree_size = subTreeSize(root->left_son)+subTreeSize(root->right_son)+1;
        root->sum= trafficSum(root->left_son)+trafficSum(root->right_son)+root->traffic;
        return root;
    }
